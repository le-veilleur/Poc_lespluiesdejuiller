...
export default function ConferencesPage() {
  ...
  // Inscription a une conference
  // 1. Appelle POST /api/planning avec le conferenceId
  // 2. Si succes : met a jour le state local (registeredCount + isRegistered)
  //    sans re-fetcher l'API → l'UI se met a jour instantanement
  // 3. Si erreur 403 : l'utilisateur n'a pas de billet
  // 4. Si erreur 409 : conference complete OU deja inscrit (message de l'API)
  async function registerForConference(conferenceId: string) {
    ...
    try {
      ...
      if (res.status === 403) {
        setMessage("Vous devez acheter un billet avant de vous inscrire");
        return;
      }

      // 409 = conference complete OU deja inscrit
      // On affiche directement le message d'erreur de l'API
      if (res.status === 409) {
      ...
      // - registeredCount +1 (une place en moins)
      // - isRegistered = true (le bouton devient le badge "Inscrit")
      setConferences((prev) =>
        prev.map((c) =>
          c.id === conferenceId
            ? {
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {conferences.map((conf) => {
            ...
            // registeredCount = nombre d'inscrits (vient de l'API via _count.planning)
            // remainingSpots = capacite totale - inscrits
            const remainingSpots = conf.capacity - (conf.registeredCount ?? 0);
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  {/* Bouton d'inscription (visible seulement si connecte) */}
                  {/* 3 etats possibles : */}
                  ...
                  {user &&
                    (conf.isRegistered ? (
                      ...
                      <button
                        onClick={() => registerForConference(conf.id)}
                        disabled={addingId === conf.id || isFull}
